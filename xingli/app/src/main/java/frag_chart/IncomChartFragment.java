...
//        获取这个月每天的支出总金额，
        List<BarChartItemBean> list = DBMangerBudget.getSumMoneyOneDayInMonth(year, month, kind);
        if (list.size() == 0) {
...
//        获取本月收入最高的一天为多少，将他设定为y轴的最大值
        float maxMoney = DBMangerBudget.getMaxMoneyOneDayInMonth(year, month, kind);
