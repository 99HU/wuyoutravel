        ...
        public void onClick(DialogInterface dialog, int which)
        {
            switch (which)
            {
                case AlertDialog.BUTTON_POSITIVE:// "确认"按钮退出程序
                    Intent intent = new Intent(MainActivity.this, PostForgetByTelActivity.class);
                    startActivity(intent);
                    break;
                case AlertDialog.BUTTON_NEGATIVE:// "取消"第二个按钮取消对话框
                    Intent intent1 = new Intent(MainActivity.this, PostForgetByEmailActivity.class);
        ...
        login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (s_account.isEmpty() || s_passWord.isEmpty()) {
                    ...
                    BmobQuery<Manager> userBmobQuery = new BmobQuery<>();
                    userBmobQuery.addWhereEqualTo("username", s_account);
                    userBmobQuery.findObjects(new FindListener<Manager>() {
                        @Override
                        public void done(List<Manager> list, BmobException e) {
                            if (list.size() != 0) {
                                final Manager manager = new Manager(s_account, s_passWord);
                                manager.login(new SaveListener<Manager>() {
                                    @Override
                                    public void done(Manager bmobPostStation, BmobException e) {
                                        if (e == null) {
                                            //账号密码正确，复选框操作
                                            editor = pref.edit();
                                            ...
                                            Toast.makeText(MainActivity.this, "欢迎" + manager.getUsername(), Toast.LENGTH_SHORT).show();

                                            Intent intent = new Intent(MainActivity.this, PostHomePage.class);
                                            startActivity(intent);
                                        } else {
